        ...
        <div class="container d-flex align-items-center justify-content-between">
            ...
            <nav id="navbar" class="navbar">
                <ul>
                    ...
                    <li> <a class=" btn btn-secondary p-2 mx-4" href="{{ url('user/message') }}">Message</a></li>
                        ...
                        <table class="table table-striped-columns">
                            ...
                            @foreach ($answer as $answer)
                                <tr>
                                    ...
                                    <td>
                                        @if ($answer->result)
                                            @foreach (json_decode($answer->result) as $result)
                                                {{ $result . ',' }}
                                            @endforeach
                                        @endif
                                    </td>
                                    <td>{{ $answer->created_at }}</td>
                                    <td><button class="btn btn-sm btn-outline-success" data-bs-toggle="modal"
                                            data-bs-target="#modalDetail{{ $answer->id }}">Detail</button></td>
                                </tr>
                                <div class="modal fade" id="modalDetail{{ $answer->id }}" tabindex="-1"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                @php $x=1 @endphp
                                                @foreach (json_decode($answer->answer) as $data)
                                                    <p>{{ $x . '. ' . $data }}</p>
                                                    @php $x++ @endphp
                                                @endforeach
                                                <h3 class="bg-warning text-white p-2 text-center">Result : @foreach (json_decode($answer->result) as $data)
                                                        {{ $data . ',' }}
                                                    @endforeach
                                                </h3>

...
class SurveiController extends Controller
{
    ...
    public function proses(Request $request)
    {
        ...
        $arr_hasil_filter = array_filter($arr_hasil, 'strlen');
        $hasil_kategori = array();
        $hasil_keterangan = array();
        $hasil_icon = array();
        // ambil data-data pada array dan masukkan sesuai dengan field
        for ($i = 0; $i < count($arr_hasil_filter); $i++) {
            array_push($hasil_kategori, $kategori[$arr_hasil_filter[$i]]->kategori);
            array_push($hasil_keterangan, $kategori[$arr_hasil_filter[$i]]->keterangan);
            array_push($hasil_icon, $kategori[$arr_hasil_filter[$i]]->icon);
        }
